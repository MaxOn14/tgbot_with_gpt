...
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, KeyboardButton, ReplyKeyboardMarkup, \
    ReplyKeyboardRemove, Update, Message, CallbackQuery
...
def get_response_from_openai():  # Define the function to get the response from the chat bot
    ...
    def get_response(user_input: str):
        ...
        try:  # Try to get the response from chatbot GPT-3.5-turbo
            ...
            response = openai.ChatCompletion.create(model=MODEL_ENGINE, messages=CONVERSATIONS, temperature=1.2,
                                                    max_tokens=2596, top_p=0.8)
            reset_data(
                CONVERSATIONS)  # The chatbot remembers its last 5 questions and 5 answers and then just the last question and answer
        ...
        try:
            ...
            current_time = show_time_now()
            write_log_to_file_txt("error_log.txt",
                                  f"An error occurred while 'extracting' the response from OpenAI, not found key 'choices' - at {current_time}: {response}\n")
            clear_data(CONVERSATIONS)  # Clear cache data
...
def chat_handler(update: Update, context: CallbackContext):  # Define the chat message handler
    ...
    write_log_to_file_txt("history_conversation.txt",
                          new_conversation_to_write_to_file_text)  # Appending new conversation to the file history conversation
...
def main():
    updater: Updater = Updater(token=TOKEN_TELEGRAM_BOT,
                               use_context=True)  # Create the Updater and pass it the bot's token
